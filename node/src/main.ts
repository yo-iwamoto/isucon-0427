...
async function validateSession(req: Request): Promise<{ valid: boolean; user?: UserRow }> {
  ...
  // BAN statusの確認
  const [[user]] = await pool.query<UserRow[]>('SELECT * FROM user where `account` = ?', [req.session.user_account]);
  if (!user || user.is_ban) {
    return {
      valid: false,
    };
...
async function getPlaylistByUlid(db: mysql.Connection, playlistUlid: string): Promise<PlaylistRow | undefined> {
  const [[row]] = await db.query<PlaylistRow[]>('SELECT * FROM playlist WHERE `ulid` = ?', [playlistUlid]);
  return row;
}
...
async function getSongsCountByPlaylistId(db: mysql.Connection, playlistId: number): Promise<number> {
  const [[row]] = await db.query<RowDataPacket[]>('SELECT COUNT(*) AS cnt FROM playlist_song where playlist_id = ?', [
    playlistId,
  ]);
  return row.cnt;
}
...
async function getRecentPlaylistSummaries(db: mysql.Connection, userAccount: string): Promise<Playlist[]> {
  const [allPlayLists] = await db.query<GetRecentPlaylistRow[]>(
    `SELECT
...
async function getFavoritedPlaylistSummariesByUserAccount(
  db: mysql.Connection,
  userAccount: string
): Promise<Playlist[]> {
  const [playlistFavorites] = await db.query<PlaylistFavoriteRow[]>(
    ...
    WHERE
      playlist.is_public = 1
      AND user.is_ban = 0
      AND playlist_favorite.favorite_user_account = ?
    GROUP BY playlist_id
    ORDER BY favorite_count DESC
    LIMIT 100;
    `,
  ...
  return Promise.all(
    playlistFavorites.map(async (row) => ({
      ulid: row.ulid,
      name: row.name,
      user_display_name: row.user_display_name,
      user_account: row.user_account,
...
async function getPlaylistDetailByPlaylistUlid(
  ...
  const resPlaylistSongs = await db.query<PlaylistSongRow[]>('SELECT * FROM playlist_song WHERE playlist_id = ?', [
    playlist.id,
  ]);
  ...
  const songs: Song[] = await Promise.all(
    playlistSongRows.map(async (row: PlaylistSongRow): Promise<Song> => {
      const [[song]] = await db.query<SongRow[]>('SELECT * FROM song WHERE id = ?', [row.song_id]);

      const [[artist]] = await db.query<ArtistRow[]>('SELECT * FROM artist WHERE id = ?', [song.artist_id]);
...
async function getPlaylistFavoritesByPlaylistIdAndUserAccount(
  db: mysql.Connection,
  playlistId: number,
...
async function getUserByAccount(db: mysql.Connection, account: string): Promise<UserRow | undefined> {
  const [result] = await db.query<UserRow[]>('SELECT * FROM user WHERE `account` = ?', [account]);
  if (!result.length) return;
...
async function insertPlaylistSong(
  ...
  await db.query('INSERT INTO playlist_song (`playlist_id`, `sort_order`, `song_id`) VALUES (?, ?, ?)', [
    arg.playlistId,
    arg.sortOrder,
    arg.songId,
  ]);
...
async function insertPlaylistFavorite(
  db: mysql.Connection,
...
app.post('/api/playlist/:playlistUlid/update', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // name, is_publicの更新
    await db.query('UPDATE playlist SET name = ?, is_public = ?, `updated_at` = ? WHERE `ulid` = ?', [
      name,
      is_public,
      updatedTimestamp,
      playlist.ulid,
...
app.post('/api/playlist/:playlistUlid/favorite', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (is_favorited) {
      // insert
      const createdTimestamp = new Date();
      const playlistFavorite = await getPlaylistFavoritesByPlaylistIdAndUserAccount(
        db,
        playlist.id,
      ...
      // delete
      await db.query('DELETE FROM playlist_favorite WHERE `playlist_id` = ? AND `favorite_user_account` = ?', [
        playlist.id,
        req.session.user_account,
      ]);
    }
...
const userLastCreatedAt: string = '2022-04-24 08:11:18.000';
const playlistLastId: number = 393165;
const playlistFavoriteLastCreatedAt: string = '2022-04-24 08:49:59.000';

app.post('/initialize', async (req: Request, res: Response) => {
  const db = await pool.getConnection();
  try {
    await db.query('DELETE FROM user WHERE ? < `created_at`', [userLastCreatedAt]);
    await db.query('DELETE FROM playlist WHERE ? < `id`', [playlistLastId]);
    await db.query('DELETE FROM playlist_song WHERE ? < `playlist_id`', [playlistLastId]);
    await db.query('DELETE FROM playlist_favorite WHERE ? < `playlist_id` OR ? <= `created_at`', [
      playlistLastId,
      playlistFavoriteLastCreatedAt,
    ]);
    const body: BasicResponse = {
      result: true,
      status: 200,
